...
class AdminKycReviewTest extends TestCase
{
    ...
    private function submitKyc(User $driver, string $licenseNumber): \Illuminate\Testing\TestResponse
    {
        $scan = $this->actingAs($driver, 'sanctum')
            ->postJson('/api/driver/kyc/scan', ['license_document' => UploadedFile::fake()->image('license.jpg')])
            ->assertOk();

        return $this->actingAs($driver, 'sanctum')->postJson('/api/driver/kyc', [
    ...
    public function test_submission_stays_submitted_in_manual_mode_by_default(): void
    {
        ...
        $this->submitKyc($driver, 'LIC-001')->assertCreated()->assertJsonPath('kyc_status', 'submitted');
    }
    ...
    public function test_submission_auto_approves_when_mode_is_automatic(): void
    {
        ...
        $this->submitKyc($driver, 'LIC-002')->assertOk()->assertJsonPath('kyc_status', 'approved');
    }
