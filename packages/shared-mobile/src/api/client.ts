...
// Each app sets its own value via app.config.js `extra.tokenStorageKey` —
// kept distinct per app (rather than a single shared literal) so existing
// installs don't get silently signed out by picking up this shared module.
const TOKEN_KEY = (Constants.expoConfig?.extra?.tokenStorageKey as string | undefined) ?? 'intercity_token';
