...
interface ThreadChatPageProps {
  /** Route param name carrying the thread's numeric id, e.g. "bookingId". */
  paramName: string;
  fetchMessages: (id: number) => Promise<Message[]>;
  sendMessage: (id: number, body: string) => Promise<Message>;
}

/**
 * Generic 1:1 chat thread UI — same shape as ChatPage (booking chat) and
 * DemLeguiChatPage, just parameterized over which endpoints to poll/post
 * to, so a new thread type (Anando, delivery) only needs its own thin
 * per-app page passing the right fetch/send functions, not a whole new
 * copy of this UI.
 */
export function ThreadChatPage({ paramName, fetchMessages, sendMessage }: ThreadChatPageProps) {
