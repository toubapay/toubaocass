...
export function AddressMapPicker({ addressLine, onAddressLineChange, latitude, longitude, onLocationChange }: Props) {
  ...
  const selectPrediction = async (prediction: Prediction) => {
    ...
    try {
      const res = await fetch(
        `https://maps.googleapis.com/maps/api/place/details/json?place_id=${prediction.placeId}&fields=geometry,formatted_address&language=${i18n.language}&key=${GOOGLE_MAPS_API_KEY}`,
      );
      const data = await res.json();
