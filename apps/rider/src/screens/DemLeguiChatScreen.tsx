...
type Props = NativeStackScreenProps<ServicesStackParamList, 'DemLeguiChat'>;

const POLL_INTERVAL_MS = 4000;

/**
 * Chat thread for a Dem Légui request — same shape as ChatScreen (booking
 * chat), just pointed at the dem-legui/requests/{id}/messages endpoints.
 */
export function DemLeguiChatScreen({ route }: Props) {
  ...
  const load = useCallback(async () => {
    try {
      const data = await fetchDemLeguiMessages(requestId);
      setMessages(data);
    } catch {
  ...
  const handleSend = async () => {
    ...
    setError(undefined);
    try {
      const message = await sendDemLeguiMessage(requestId, trimmed);
