...
function chatUrl(chat: ActiveChat): string {
  return chat.type === 'booking' ? `/chat/${chat.id}` : `/dem-legui/requests/${chat.id}/chat`;
}

/**
 * Floating shortcut to the driver's single most relevant conversation, plus
 ...
 * specific trip's booking list, with nothing surfacing new messages while
 * browsing elsewhere. Deliberately independent of push notifications
 * (which need FCM configured and permission granted) so it always works.
 */
