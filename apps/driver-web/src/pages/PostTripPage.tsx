...
export function PostTripPage() {
  ...
  const canSubmit = kycApproved && carId && origin && destination && origin.id !== destination.id && date && time && Number(fare) > 0;
  ...
  return (
    <div>
      ...
      <h1 style={{ fontSize: 24, fontWeight: 700, color: colors.text, marginBottom: spacing.md }}>{t('trips.post.title')}</h1>
      ...
      {!kycApproved ? (
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Button label={t('trips.post.submit')} type="submit" disabled={!canSubmit} loading={loading} />
        </form>
      )}
    </div>
  );
