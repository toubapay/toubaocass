...
export function AnandoPage() {
  ...
  useEffect(() => {
    ...
    // stays current without a manual reload.
    ...
    <div>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: spacing.sm }}>
            ...
            <Button
              ...
              onClick={() => navigate(`/anando/${activeRide.id}`)}
              variant="outline"
              style={{ flex: 1 }}
            />
          </div>
      ...
      {tab === 'available' ? (
        <>
          {loading ? (
            ...
            rides.map((ride) => <RideCard key={ride.id} ride={ride} onClick={() => navigate(`/anando/${ride.id}`)} />)
          )}
        </>
      ) : (
        <>
          ...
          {myRides.length === 0 ? (
            ...
            <div style={{ marginBottom: spacing.lg }}>
              {myRides.map((ride) => (
                <RideCard key={ride.id} ride={ride} onClick={() => navigate(`/anando/${ride.id}`)} />
              ))}
            </div>
          )}
          ...
          {myBookings.length === 0 ? (
            ...
            myBookings.map((booking) => (
              <BookingCard key={booking.id} booking={booking} onClick={() => navigate(`/anando/${booking.anando_ride.id}`)} />
            ))
          )}
        </>
      )}
      ...
      {postedRideId != null && (
        <SuccessModal
          ...
          onClose={() => navigate(`/anando/${postedRideId}`)}
        />
      )}
    </div>
  );
