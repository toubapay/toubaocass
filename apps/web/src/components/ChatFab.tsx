...
/**
 * Floating shortcut to the rider's single most relevant conversation (most
 * recent incoming message, or most recent active booking/Dem Légui request
 * if nobody's spoken yet) — chat was otherwise only reachable by drilling
 * into a specific trip/request. Hides itself on chat screens themselves and
 * whenever there's nothing to jump to.
 */
export function ChatFab() {
  ...
  const handleClick = () => {
    if (chat.type === 'booking') {
      ...
      navigate(`/services/dem-legui/${chat.id}/chat`);
    }
  };
